...
public class LevelManager : MonoBehaviour
{
	...
	bool hasDisplayedPlayerStartError = false;
	...
	void SceneLoaded(Scene scene, LoadSceneMode loadingMode)
	{
		...
		{
			GameObject.FindWithTag("Player").GetComponent<ObjInfo>().LOAD();
	...
	void SetPlayerJumpPoints()
	{
		SetPlayerStart();	// also throws an error if > 1
	...
	void SetPlayerStart()
	{
		...
		if (starts.Length > 1 && !hasDisplayedPlayerStartError)
		{
			//Debug.LogError("You have more than one object tagged 'PlayerStart'. You should only have one.");
			hasDisplayedPlayerStartError = true;
		}
		...
		if (player != null)
			SetCheckpointIfNone(player);
	}

	void SetCheckpointIfNone(GameObject player)
	{
		...
		if (playerHandler.LastCheckpoint == null && !playerHandler.HasRespawnPos)
		{
			// don't set startpos to player start location when scene base is loaded,
			// otherwise, player startpos will get set to SCENEBASE weird position
			if (playerStartPoint == null && SceneBaseLoaded)
				return;

			Transform start = (playerStartPoint != null) ? playerStartPoint : player.transform;
			playerHandler.SetCheckpoint(start.position);
		}
	}
